...
For the price class, I chose the "middle" option (North America, Europe, Asia, Middle East, and Africa), but this depends on your needs. For now, I don't need a Web Application Firewall (WAF) or any geographic restrictions.
...
#+begin_comment
TODO: Add a section with logging setup
#+end_comment

Setting origins depends on the S3 setup you choose. In the case of *Static Website Hosting*, the orgin domain should be set to the /S3 website endpoint/. For a private S3 bucket, the origin domain shoud be the /S3 endpoint/. Additionally, you need to create an *Orgin Access Control (OAC)* and attach it in the /Orgin access/ settings. For connection attempts, connection timeout, and respones timeout, I kept the default values (*3*, *10*, *30*).

I'm using only the defualt behaviour: *compression enabled*, *HTTP redirected to HTTPS*, and only *GET* and *HEAD* methods allowed. There are no restrictions for viewer access. I'm using the recommended *CashingOpimized* cache policy, with no origin request policy or response headers policy configured. If you use a private bucket, handling "nice URLs" (without /index.html/) must be done using a [[https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/example_cloudfront_functions_url_rewrite_single_page_apps_section.html][Viewer request function]].
...
One of the nice features of this setup is setting a budged ([[#references][5]]). I haven't set up any automatic actions of the budget is exceeded, but at least I receive an email notification when the costs pass the treshold. It's not a perfect solution, but at least i get feedback about my spending and can take action before the situation gets out of hand.
